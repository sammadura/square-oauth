...
def dashboard():
    ...
    <style>
        ...
        .logs { background: #f8f9fa; padding: 10px; border-radius: 5px; font-family: monospace; font-size: 12px; margin: 10px 0; }
    </style>
    ...
    <div style="background: #e9ecef; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
        ...
        <p><strong>Logging:</strong> Enhanced debugging enabled</p>
    </div>
    ...
    <div style="margin-top: 20px; padding: 15px; background: #e7f3ff; border-radius: 8px;">
        ...
        <div class="logs">
            ...
            • Check Render logs for detailed sync information<br>
            • Use Debug button to check all connections<br>
            • Manual Sync shows step-by-step progress<br>
    ...
    try:
        ...
        if success:
            ...
            <div style="max-width: 600px; margin: 50px auto; padding: 30px; background: white; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); font-family: Arial, sans-serif;">
                ...
                <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin: 20px 0; font-size: 12px;">
                    <strong>💡 Tip:</strong> Check your Render logs for detailed sync information including step-by-step progress and any warnings.
                </div>
            </div>
            '''
        else:
            ...
            <div style="max-width: 600px; margin: 50px auto; padding: 30px; background: white; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); font-family: Arial, sans-serif;">
                ...
                <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin: 20px 0; font-size: 12px;">
                    <strong>💡 Debugging:</strong> Check your Render application logs for detailed error information. The logs will show exactly where the sync failed.
                </div>
            </div>
            '''
        ...
        <div style="max-width: 600px; margin: 50px auto; padding: 30px; background: white; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); font-family: Arial, sans-serif;">
            ...
            <div style="background: #f8d7da; padding: 20px; border-radius: 8px; margin: 20px 0;">
                ...
                <p><strong>Check Render logs for full error details.</strong></p>
            </div>
        ...
        if response.status_code == 200:
            ...
            if save_tokens_to_sheets(merchant_id, new_access_token, new_refresh_token, tokens.get('merchant_name')):
                ...
                if success:
                    ...
                    <p><strong>💡 Check Render logs for detailed error information</strong></p>
                    <br>
...
def force_sync_all():
    ...
    <p><strong>💡 Check Render logs for detailed sync information</strong></p>
    <a href="/dashboard">Back to Dashboard</a>
    '''
...
def export_customers(merchant_id):
    ...
    try:
        print(f"📊 CSV export requested for merchant {merchant_id}")
        ...
        try:
            ...
            if not data:
                return f'No customer data found for merchant {merchant_id}. Try syncing first.', 404
            
            print(f"✅ Retrieved {len(data)} rows for export")
            ...
            print(f"❌ Customer sheet not found: {e}")
            return f'Customer data not found for merchant {merchant_id}. Try running a sync first.', 404
...
def debug_merchant(merchant_id):
    ...
    if tokens:
        try:
            ...
            if response.status_code == 200:
                ...
                # Test invoice API
                invoice_response = requests.post('https://connect.squareupsandbox.com/v2/invoices/search', headers=headers, json={'limit': 5})
                if invoice_response.status_code == 200:
                    invoice_data = invoice_response.json()
                    invoice_count = len(invoice_data.get('invoices', []))
                    debug_info['checks']['invoice_data'] = f'✅ Found {invoice_count} invoices (access granted)'
                else:
                    debug_info['checks']['invoice_data'] = f'⚠️ Invoice API: {invoice_response.status_code} (may not have invoice permissions)'
            else:
    ...
    <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h3>📊 Quick Summary</h3>
        <p><strong>Merchant Name:</strong> {debug_info.get('merchant_name', 'Unknown')}</p>
        <p><strong>Last Sync:</strong> {debug_info.get('last_sync', 'Never')}</p>
        <p><strong>Total Customers:</strong> {debug_info.get('total_customers', 0)}</p>
        <p><strong>Timestamp:</strong> {debug_info['timestamp']}</p>
    </div>
    
    <pre style="background: #f8f9fa; padding: 15px; border-radius: 8px; overflow-x: auto; font-size: 12px;">{json.dumps(debug_info, indent=2, default=str)}</pre>
    
    <div style="margin: 20px 0;">
        <a href="/api/sync/{merchant_id}" style="background: #28a745; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin: 0 10px;">🔄 Try Manual Sync</a>
        <a href="/dashboard" style="background: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin: 0 10px;">📊 Back to Dashboard</a>
    </div>
    
    <div style="background: #fff3cd; padding: 15px; border-radius: 8px; margin: 20px 0;">
        <strong>💡 Debugging Tips:</strong><br>
        • All checks should show ✅ for successful sync<br>
        • Check Render logs for detailed error messages<br>
        • Invoice permissions are optional but recommended<br>
        • Customer sheet will be created after first successful sync
    </div>
    """
...
def health():
    return jsonify({
        ...
        'timestamp': datetime.now().isoformat(),
        'version': '2.0.0',
        'features': ['customer_sync', 'invoice_integration', 'comprehensive_logging']
    })
